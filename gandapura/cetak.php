...
$maxClusters = $_GET['cluster'];
  ...
  $pecah = mysqli_fetch_array($data);
    ...
    $alamat = mysqli_query($koneksi, "SELECT alamat FROM gandapura GROUP BY alamat");
    while($row_alamat = mysqli_fetch_assoc($alamat)) {
        $jk = mysqli_query($koneksi, "SELECT jenis_kelamin FROM gandapura GROUP BY jenis_kelamin");
        while($row_jk = mysqli_fetch_assoc($jk)) {
            $read = mysqli_query($koneksi, "SELECT * FROM gandapura WHERE alamat='$row_alamat[alamat]' AND jenis_kelamin='$row_jk[jenis_kelamin]' ");
            $batas = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM gandapura WHERE alamat='$row_alamat[alamat]' AND jenis_kelamin='$row_jk[jenis_kelamin]' "));
            $tampung = array();
...
?>
        ...
        <div class="kopSurat">
            ...
            <h1 style="margin-top:-150px; margin-bottom:140px; margin-left:255px; font-size:50px;" position="fixed">PUSKESMAS GANDAPURA</h1>
            <h3 style="margin-top:-130px; margin-bottom:90px; margin-left:260px; font-size:13px;">Jl. Banda Aceh-Medan Km. 550 Desa Lapang Timu Kecamatan Gandapura (269,04 km) Bireuen, Aceh 24356</h3>
        </div>
        <hr>
